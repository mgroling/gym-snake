...
class SnakeEnv(gym.Env):
    ...
    def __init__(
        ...
        ), "Invalid parameter: size must be an integer and greater than zero"
        try:
            assert (
                len(reward) >= 4
            ), "Invalid parameter: reward must be iterable and contain four or more elements, (all numbers)"
        except:
            assert (
                False
            ), "Invalid parameter: reward must be iterable and contain four or more elements, (all numbers)"
        assert obs in [
            ...
            "ray",
            "simple",
        ], 'Invalid parameter: obs must be either "image", "ray" or "simple"'
        assert spawn in [
            "center",
            "random",
        ], 'Invalid parameter: spawn must be either "center" or "random"'
        assert (
            type(add_len) == int and add_len >= 0
        ), "Invalid parameter: add_len must be an integer and greater-or-equal than zero"
        assert (
            type(start_length) == int and start_length > 0
        ), "Invalid parameter: start_length must be an integer and gerater than zero"
        assert (
            type(termination) == int and termination > 0
        ), "Invalid parameter: termination must be an integer and greater than zero"
        ...
        if obs == "image":
            self.observation_space = gym.spaces.Box(
                np.zeros((self._size, self._size)), np.full((self._size, self._size), 2)
            )
        elif obs == "ray":
    ...
    def reset(self):
        ...
        if self._spawn == "center":
            ...
            for i in range(0, self._start_len):
                self._snake.append(
                    (self._size // 2, self._size // 2 - self._start_len // 2 + i)
                )
        elif self._spawn == "random":
            self._snake = [
                ((np.random.randint(self._size), np.random.randint(self._size)))
            ]
            for i in range(1, self._start_len):
                direction = self._act_mapping[np.random.randint(4)]
                next_part = np.array(
                    [
                        self._snake[-1][0] + direction[0],
                        self._snake[-1][1] + direction[1],
                    ]
                )
            ...
            self._squares = [
                [
                    rendering.FilledPolygon(
                        [
                            (i * length_square + 1, j * length_square + 1),
                            ((i + 1) * length_square - 1, j * length_square + 1),
                            ((i + 1) * length_square - 1, (j + 1) * length_square - 1),
                            (i * length_square + 1, (j + 1) * length_square - 1),
                        ]
                    )
    ...
    def _spawnApple(self):
        ...
        while self._matrix[rand_x, rand_y] != 0:
            rand_x, rand_y = np.random.randint(self._size), np.random.randint(
                self._size
            )
    ...
    def _getSimple(self):
        dist_apple_x_y = np.array(
            [
                1 - (self._apple[0] - self._snake[0][0]) / (self._size - 1),
                1 - (self._apple[1] - self._snake[0][1]) / (self._size - 1),
            ]
        )
        ...
        for i, j in self._mov_possibilites:
            if (
                head[0] + i >= 0
                and head[1] + j >= 0
                and head[0] + i < self._size
                and head[1] + j < self._size
            ):
                if (
                    self._matrix[head[0] + i, head[1] + j] == 0
                    or self._matrix[head[0] + i, head[1] + j] == 2
                ):
                    next_view[index] = 0
                else:
                    next_view[index] = 1
